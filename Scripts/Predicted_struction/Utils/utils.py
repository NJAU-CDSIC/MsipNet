...
STRUCTURE_CLASSES_RNAfold = ['E', 'L', 'R', 'H', 'T', 'B', 'M']


def RNAfold_to_onehot(structures):
    num_sequences = len(structures)
    max_len = max(len(struct) for struct in structures)
    num_classes = len(STRUCTURE_CLASSES_RNAfold)

    onehot_array = np.zeros((num_sequences, num_classes, max_len), dtype=np.int8)

    for i, struct in enumerate(structures):
        for j, char in enumerate(struct):
            if char in STRUCTURE_CLASSES_RNAfold:
                class_idx = STRUCTURE_CLASSES_RNAfold.index(char)
                onehot_array[i, class_idx, j] = 1

    one_hot = np.array(onehot_array)
    return one_hot


def read_fasta_with_RNAfold(filename):
    sequences = []
    structures = []
    labels = []
    ...
    with open(filename, 'r') as f:
        ...
        while i < len(lines):
            ...
            if line.startswith('>'):
                ...
                structures.append(struct_line)

                i += 1
            else:
                i += 1

    return sequences, structures, labels

structure_classes_RNAshapes = ['F', 'H', 'I', 'M', 'S', 'T']
num_classes = len(structure_classes_RNAshapes)


def RNAshapes_to_onehot(structures):
    max_len = max(len(struct) for struct in structures)
    num_sequences = len(structures)

    onehot_array = np.zeros((num_sequences, num_classes, max_len), dtype=int)

    for i, struct in enumerate(structures):
        for j, char in enumerate(struct):
            try:
                class_idx = structure_classes_RNAshapes.index(char)
                onehot_array[i, class_idx, j] = 1
            except ValueError:
                pass
...
def read_fasta_with_RNAshapes(filename):
    sequences = []
    structures = []
    labels = []
...
class myDataset(Dataset):
    ...
    Stores RNA-FM embeddings, secondary structure, one-hot encodings, and labels.
    __getitem__ returns a tuple of (embedding, structure, one-hot, label) for a given index.
    __len__ returns the total number of samples in the dataset.
    Suitable for use with DataLoader for batch training and evaluation.
    """

    def __init__(self, bert_embedding, structure, onehot, label):
    ...
    def __getitem__(self, index):
        ...
        return embedding, struct, hot, label
